...
def test_dir_check():
	"""
	Testing checking if a directory is empty
	"""
	is_empty = utils.check_dir_empty('tests')
	assert not is_empty

	tmp_dir = 'tmp/test_dir'
	utils.create_dir(tmp_dir)
	is_empty = utils.check_dir_empty(tmp_dir)
	assert is_empty
	os.rmdir(tmp_dir)
...
def test_is_valid_image():
	"""
	Testing checking if a file is a valid image
	"""
